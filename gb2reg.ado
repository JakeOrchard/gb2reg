...
version 13.0
	if replay() {
		...
		syntax varlist(min=1) [if] [in] [, QINFinite LNormal sigma(varlist) p(varlist) q(varlist)  INITial(numlist) CONSTraints(passthru) DIFficult TECHnique(passthru) ITERate(passthru) nolog TRace GRADient showstep HESSian SHOWTOLerance TOLerance(passthru) LTOLerance(passthru) NRTOLerance(passthru)] 
	...
	*entries and issues a warning.
	...
	if "`initial'" != ""{
		...
		if "`lnormal'" == ""{
			 ...
			 if "`qinfinite'" == "" {
				local evaluator llf_gb2
				ml model lf `evaluator' (delta: `depvar' = `regs')  (sigma: `sigmavars')  (p: `pvars') (q: `qvars') if `touse', search(on) maximize `constraints' `technique'  `difficult' `iterate' `log' `trace' `gradient' `showstep' `hessian' `showtolerance' `tolerance' `ltolerance' `nrtolerance' title(`gb2title')
			}
			else {
				local evaluator llf_gg
				ml model lf `evaluator' (delta: `depvar' = `regs')  (sigma: `sigmavars')  (p: `pvars') if `touse', search(on) maximize `constraints' `technique'  `difficult' `iterate' `log' `trace' `gradient' `showstep' `hessian' `showtolerance' `tolerance' `ltolerance' `nrtolerance'	 title(`ggtitle')		
			}
		}
		else {
			local evaluator llf_ln
			ml model lf `evaluator' (delta: `depvar' = `regs') (sigma: `sigmavars')  if `touse', search(on) maximize `constraints' `technique' `difficult' `iterate' `log'  `trace' `gradient' `showstep' `hessian' `showtolerance' `tolerance' `ltolerance' `nrtolerance'		title(`lntitle')	
